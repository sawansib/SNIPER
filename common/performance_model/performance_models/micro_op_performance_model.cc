...
void MicroOpPerformanceModel::handleInstruction(DynamicInstruction *dynins)
{
   ...
   else
   {
	...
	m_current_uops[exec_base_index]->setXchgRegValue(dynins->reg_value);
      ...
      if (dynins->is_marker){
	//std::cout<<dynins->is_marker_begin_loop<<"\n";
	...
	m_current_uops[exec_base_index]->mSetMarkerBeginLoop(dynins->is_marker_begin_loop);
	m_current_uops[exec_base_index]->mSetMarkerEndLoop(dynins->is_marker_end_loop);
      }
   #if DEBUG_INSN_LOG
      if (insn_cost > 17)
      {
