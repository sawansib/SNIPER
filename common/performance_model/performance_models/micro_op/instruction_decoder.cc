...
const std::vector<const MicroOp*>* InstructionDecoder::decode(IntPtr address, const xed_decoded_inst_t *ins, Instruction *ins_ptr)
{
   ...
   for(int index = 0; index < totalMicroOps; ++index)
   {
      ...
      else if (index < numLoads + numExecs) /* EXEC */
      {
         ...
         // Double check for deptrace
         if (xed_decoded_inst_get_category(ins) == XED_CATEGORY_COND_BR)
         {
            assert(numLoads == 0 && numStores == 0);
         }
      ...
      // If we are a load, and there is also a store, make sure that we mark this as Last as well to increment the instruction count artificially (for deptrace)
      if (index < numLoads) /* LOAD */
      {
         if (numStores)
         {
         ...
         // For the two-load case, mark the first load in addition
         if (numLoads > 1)
         {
            if (index == 0)
            {
