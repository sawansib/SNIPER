...
Core::Core(SInt32 id)
   ...
   , m_thread(NULL)
   , m_bbv(id)
   , m_light_caches(Sim()->getConfig()->getLightCacheNum(), NULL)
   ...
   , m_instructions_hpi_last(0)
{
   ...
   for (UInt32 i = 0 ; i < Sim()->getConfig()->getLightCacheNum() ; ++i)
      m_light_caches[i] = new LightCache(id, i);
}
      ...
      SubsecondTime now)
{
   ...
   UInt64 cache_block_size = getMemoryManager()->getCacheBlockSize();
