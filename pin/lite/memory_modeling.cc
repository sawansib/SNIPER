...
{
   UInt32 num_lightcaches = Sim()->getConfig()->getLightCacheNum();
   if (num_lightcaches)
   {
      if (INS_IsMemoryRead (ins) || INS_IsMemoryWrite (ins))
      {
         ...
         {
            if ((num_lightcaches > 1) & INSTR_IF_NOT_FASTFORWARD(inst_mode))
            {
               INS_InsertPredicatedCall(ins, IPOINT_BEFORE, AFUNPTR(lite::lightMemoryModel2),
                  IARG_THREAD_ID,
                  IARG_BOOL, INS_MemoryOperandIsWritten(ins, i) || INS_IsAtomicUpdate(ins),
                  IARG_MEMORYOP_EA, i,
                  IARG_END);
            }
            else
            {
               INS_InsertPredicatedCall(ins, IPOINT_BEFORE, AFUNPTR(lite::lightMemoryModel),
                  IARG_THREAD_ID,
                  IARG_BOOL, INS_MemoryOperandIsWritten(ins, i) || INS_IsAtomicUpdate(ins),
                  IARG_MEMORYOP_EA, i,
                  IARG_END);
...
{
   localStore[thread_id].thread->getCore()->getLightCache()->access(eip, write);
}
...
void lightMemoryModel2(THREADID thread_id, BOOL write, ADDRINT eip)
{
   ...
   core->getLightCache(0)->access(eip, write);
   core->getLightCache(1)->access(eip, write);
}
