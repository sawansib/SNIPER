...
do
	...
	(
		cd Python-2.7.2
		schroot -p -csl-5.5 -- ./configure --prefix=$TMPDIR/python_kit --enable-shared --without-threads --without-signal-module
		sed -i -e 's/#define HAVE_FORKPTY 1//' pyconfig.h
		sed -i -e 's/#define HAVE_OPENPTY 1//' pyconfig.h
		schroot -p -csl-5.5 -- make install
	)
	...
	echo
	echo scp $PWD_ORIG/sniper-python27-$ARCH.tgz intel@vtuin:~/mirror/
	echo scp $PWD_ORIG/sniper-python27-$ARCH.tgz snipersim@snipersim.com:snipersim.com/packages/
	echo
	echo 'Test archives and upload using the commands above, then press Enter to continue'
	read
done
