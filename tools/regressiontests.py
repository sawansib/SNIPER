...
TESTS = {
  'daily': {
    ...
    'public': [ # Shouldn't be too many commits so it won't actually run every day, but we want quick feedback
      Test('gainestown-interval', 'splash2-fft', 'large',  1, config = ('gainestown',)),
      Test('gainestown-interval', 'splash2-fft', 'large',  8, config = ('gainestown',)),
      Test('gainestown-interval', 'splash2-radix', 'large',  1, config = ('gainestown',)),
      Test('gainestown-interval', 'splash2-radix', 'large',  8, config = ('gainestown',)),
      Test('gainestown-interval', 'parsec-swaptions', 'large', 8, config = ('gainestown',)),
    ],
    'stable': [
    ]
  },
...
for bm in benchmarks_splash + benchmarks_parsec + benchmarks_corevalidation:
  for arch, maxthreads in (('eb-2-02', 8),):
    for nthreads in (1, maxthreads):
      if nthreads == 1 and bm.startswith('parsec'): continue
