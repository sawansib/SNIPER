...
# file naming scheme.  Print either just the file name and weight or a
# selected subset of all the fields.
#
# $Id: pb_weight.py,v 1.2 2014/06/02 02:59:43 tmstall Exp tmstall $
...
def GetOptions():
    ...
    @return options, args
    """
    version = '$Revision: 1.2 $';      version = version.replace('$Revision: ', '')
    ver = version.replace(' $', '')
...
for name in args:
    ...
    if options.all:
        ...
        #   file, TID, region_num, warmup, prolog, region, epilog, weight
        #
        msg.PrintMsg(all_fmt.format(name, tid, region_num, warmup, prolog, region, epilog, weight))
    else:
